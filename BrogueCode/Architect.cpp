...
bool checkLoopiness(short x, short y) {
    ...
    for (sdir = 0; sdir < DIRECTION_COUNT; NEXT_DIR(sdir)) {
        newX = x + cDirs[sdir][0];
        newY = y + cDirs[sdir][1];
    ...
    inString = false;
    for (dir = sdir; dir < sdir + 8; NEXT_DIR(dir)) {
...
bool addTileToMachineInteriorAndIterate(char interior[DCOLS][DROWS], short startX, short startY) {
    ...
    for (dir = 0; dir < 4 && goodSoFar; NEXT_DIR(dir)) {
        newX = startX + nbDirs[dir][0];
        newY = startY + nbDirs[dir][1];
        if (coordinatesAreInMap(newX, newY)) {
...
bool blueprintQualifies(short i, unsigned long requiredMachineFlags) {
    if (getBlueprintCatalog()[i].depthRange[0] > rogue.depthLevel
        ...
        || (~(getBlueprintCatalog()[i].flags) & requiredMachineFlags)
                // May NOT have BP_ADOPT_ITEM unless that flag is required:
        || (getBlueprintCatalog()[i].flags & BP_ADOPT_ITEM & ~requiredMachineFlags)
                // May NOT have BP_VESTIBULE unless that flag is required:
        || (getBlueprintCatalog()[i].flags & BP_VESTIBULE & ~requiredMachineFlags)) {
        ...
        for(i=1; i<DCOLS-1; i++) {
            for(j=1; j < DROWS-1; j++) {
                if (cellHasTerrainFlag(i, j, T_PATHING_BLOCKER)
                    ...
                    for (nbcount = dir = (directions)0; dir < DIRECTION_COUNT; NEXT_DIR(dir)) {
                        newX = i + nbDirs[dir][0];
                        newY = j + nbDirs[dir][1];
                        if (interior[newX][newY]
                            && !cellHasTerrainFlag(newX, newY, T_PATHING_BLOCKER)) {
...
bool fillInteriorForVestibuleMachine(char interior[DCOLS][DROWS], short bp, short originX, short originY) {
    ...
    totalFreq = rand_range(getBlueprintCatalog()[bp].roomSize[0], getBlueprintCatalog()[bp].roomSize[1]); // Keeps track of the goal size.
    ...
    if ((getBlueprintCatalog()[bp].flags & BP_TREAT_AS_BLOCKING)
        && levelIsDisconnectedWithBlockingMap(interior, false)) {
        success = false;
    } else if ((getBlueprintCatalog()[bp].flags & BP_REQUIRE_BLOCKING)
                ...
                if (interior[i][j] && !(pmap[i][j].flags & IS_GATE_SITE)) {
                    for (dir=(directions)0; dir< DIRECTION_COUNT; NEXT_DIR(dir)) {
                        ...
                        if (coordinatesAreInMap(newX, newY)
                            ...
                            && cellHasTerrainFlag(newX, newY, T_PATHING_BLOCKER)) {
                            for (layer=(dungeonLayers)0; layer<NUMBER_TERRAIN_LAYERS; NEXT_LAYER(layer)) {
                                pmap[newX][newY].layers[layer] = (layer == DUNGEON ? WALL : NOTHING);
                            }
                        }
                    }
                }
...
bool buildAMachine(enum machineTypes bp,
    ...
    **distanceMap, distance25, distance75, distances[100], distanceBound[2],
    ...
    do {
        ...
        if (chooseBP) { // If no blueprint is given, then pick one:
            ...
            for (i=1; i<NUMBER_BLUEPRINTS; i++) {
                if (blueprintQualifies(i, requiredMachineFlags)) {
                    totalFreq += getBlueprintCatalog()[i].frequency;
                }
            }
            ...
            for (i=1; i<NUMBER_BLUEPRINTS; i++) {
                if (blueprintQualifies(i, requiredMachineFlags)) {
                    if (randIndex <= getBlueprintCatalog()[i].frequency) {
                        bp = (machineTypes)i;
                        break;
                    } else {
                        randIndex -= getBlueprintCatalog()[i].frequency;
                    }
                }
            }
        ...
        if (getBlueprintCatalog()[bp].flags & BP_ROOM) {
                ...
                for(i=0; i<DCOLS; i++) {
                    for(j=0; j<DROWS && totalFreq < 50; j++) {
                        if ((pmap[i][j].flags & IS_GATE_SITE)
                            ...
                            && chokeMap[i][j] >= getBlueprintCatalog()[bp].roomSize[0]
                            && chokeMap[i][j] <= getBlueprintCatalog()[bp].roomSize[1]) {
        ...
        } else if (getBlueprintCatalog()[bp].flags & BP_VESTIBULE) {
            if (chooseLocation) {
    ...
    prepareInteriorWithMachineFlags(interior, originX, originY, getBlueprintCatalog()[bp].flags, getBlueprintCatalog()[bp].dungeonProfileType);
    ...
    for(i=0; i<DCOLS; i++) {
        for(j=0; j<DROWS; j++) {
            if (interior[i][j]) {
                pmap[i][j].flags |= ((getBlueprintCatalog()[bp].flags & BP_ROOM) ? IS_IN_ROOM_MACHINE : IS_IN_AREA_MACHINE);
...
//  DEBUG logBuffer(interior);
    ...
    for (i=0; i<getBlueprintCatalog()[bp].featureCount; i++) {
        skipFeature[i] = false;
    }
    for (j = 0; j <= 1; j++) {
        totalFreq = 0;
        for (i=0; i<getBlueprintCatalog()[bp].featureCount; i++) {
            if (getBlueprintCatalog()[bp].feature[i].flags & alternativeFlags[j]) {
                skipFeature[i] = true;
                totalFreq++;
            }
        }
    ...
    for (feat = 0; feat < getBlueprintCatalog()[bp].featureCount; feat++) {
        
        if (skipFeature[feat]) {
            continue; // Skip the alternative features that were not selected for building.
        }
        
        feature = &(getBlueprintCatalog()[bp].feature[feat]);
        ...
        do { // If the MF_REPEAT_UNTIL_NO_PROGRESS flag is set, repeat until we fail to build the required number of instances.
            ...
            for(i=0; i<DCOLS; i++) {
                for(j=0; j<DROWS; j++) {
                    if (cellIsFeatureCandidate(i, j,
                                               ...
                                               machineNumber, feature->flags, getBlueprintCatalog()[bp].flags)) {
                        qualifyingTileCount++;
                        candidates[i][j] = true;
                    } else {
            ...
            for (instance = 0; (generateEverywhere || instance < instanceCount) && qualifyingTileCount > 0;) {
                ...
                if (DFSucceeded && terrainSucceeded) { // Proceed only if the terrain stuff for this instance succeeded.
                    ...
                    //if (!(getBlueprintCatalog()[bp].flags & BP_NO_INTERIOR_FLAG)) {
                    pmap[featX][featY].flags |= ((getBlueprintCatalog()[bp].flags & BP_ROOM) ? IS_IN_ROOM_MACHINE : IS_IN_AREA_MACHINE);
                    ...
                    if ((feature->flags & MF_GENERATE_ITEM)
                        || (adoptiveItem && (feature->flags & MF_ADOPT_ITEM) && (getBlueprintCatalog()[bp].flags & BP_ADOPT_ITEM))) {
                        // Are we adopting an item instead of generating one?
                        if (adoptiveItem && (feature->flags & MF_ADOPT_ITEM) && (getBlueprintCatalog()[bp].flags & BP_ADOPT_ITEM)) {
                    ...
                    if (feature->flags & (MF_OUTSOURCE_ITEM_TO_MACHINE | MF_BUILD_VESTIBULE)) {
                        ...
                        for (i=10; i > 0; i--) {
                            ...
                            if ((feature->flags & MF_OUTSOURCE_ITEM_TO_MACHINE) && theItem) {
                                ...
                                success = buildAMachine(MT_NONE, -1, -1, BP_ADOPT_ITEM, theItem, spawnedItemsSub, spawnedMonstersSub);
                            } else if (feature->flags & MF_BUILD_VESTIBULE) {
                                success = buildAMachine(MT_NONE, featX, featY, BP_VESTIBULE, NULL, spawnedItemsSub, spawnedMonstersSub);
                            }
...
void addMachines() {
    ...
    for (failsafe = 50; machineCount && failsafe; failsafe--) {
        if (buildAMachine(MT_NONE, -1, -1, BP_REWARD, NULL, NULL, NULL)) {
            machineCount--;
    ...
    if (doorSites) {
        ...
        if (attachHallway) {
            ...
            for (i=0; doorSites[dir][0] == -1 && i < 3; i++) {
                dir = (directions)(((int)dir + 1) % 4); // Each room will have at least 2 valid directions for doors.
            }
    ...
    for (i=0; i<DCOLS; i++) {
        for (j=0; j<DROWS; j++) {
            if (pmap[i][j].layers[DUNGEON] == GRANITE) {
                foundExposure = false;
                for (dir = (directions)0; dir < (includingDiagonals ? 8 : 4) && !foundExposure; NEXT_DIR(dir)) {
                    x1 = i + nbDirs[dir][0];
                    y1 = j + nbDirs[dir][1];
    ...
    if (monst->status[STATUS_ENTERS_LEVEL_IN] > 0) {
        ...
        if (theMap) {
            ...
            for (i=0; i < turnCount; i++) {
                if ((dir = (directions)nextStep(theMap, monst->xLoc, monst->yLoc, NULL, true)) != NO_DIRECTION) {
